...
if not startFromCheckpoint:
    ...
    if not loadData:
        ...
        for ticket in my_data:
            ...
            for pre_ticket in pre_my_data:
                if pre_ticket["uhd_NR"] == ticket["number"]:
                    ...
                    ticket["clean_problem"] = pre_ticket["question"]
                    # NEEDS TO BE UPDATED to clean_problem
                    loadfound += 1
                    print(loadfound)
                    found = True
                    break;
            if not found:
                # Bert Highlighting anwenden, falls nicht manuell gelabelt
                n_format, t_Format = get_bert_highlighting(ticket["clean_problem"])
                ticket["markupTFormat"] = t_Format
                ticket["markupNFormat"] = n_format
            ticket["clusterCat"] = ticket["merged_clusters"]
            # Erstelle Context-Format für VW 
            ticket["contextString"] = vw_format_creator.createContext(ticket["markupTFormat"])
            ticket['positiveRewards'] = 0
        del pre_my_data
        save_df = pd.read_json(json.dumps(my_data))
        save_df.to_excel("all_labeled_data.xlsx")
        exit(-1)
        # Perform Data Split
        ...
        all_cats_sol_count = {}
        for t in my_data:
            all_cats_sol_count[t["clusterCat"]] = 0
        counter = 0
        while counter < len(my_data):
            t = my_data[counter]
            if all_cats_sol_count[t["clusterCat"]] < sol_size * t["SetSize"] \
                    and all_cats_sol_count[t["clusterCat"]] < sol_size_max \
                    or (sol_size_min > all_cats_sol_count[t["clusterCat"]]):
                all_cats_sol_count[t["clusterCat"]] += 1
                sol_data.append(my_data.pop(counter))
            else:
                counter += 1
        ...
        with open("sol_data.json", "w", encoding="utf8") as f:
            json.dump(sol_data, f)
        a = pd.read_json("sol_data.json")
        a.to_excel("sol_data.xlsx")

    else:
        with open("train_data.json", "r", encoding="utf8") as f:
            my_data = json.load(f)
        with open("eval_data.json", "r", encoding="utf8") as f:
            eval_data = json.load(f)
        with open("sol_data.json", "r", encoding="utf8") as f:
            sol_data = json.load(f)

    LEARN = True

    if LEARN:
        vw = vowpalwabbit.pyvw.vw("--ccb_explore_adf -b 24 --interactions :: --epsilon 0.25")
    else:
    ...
    iterations = [0]
    pred_value_iteration = [[0, 0, 0, 0, 0, 0, 0, 0]]
    iteration = 0
    NUMBER_OF_SLOTS = 1
    actions = ""
    lowestPositiveReward = 0
    rewards_zero = len(my_data)
    stop_lowest_positive_Reward = 3
    ...
    actions_example = ""
    print (my_data[0]["contextString"])


    for t in sol_data:
        actions_example += "ccb action" + t["contextString"]

else:
    with open('checkpoints/pickle_' + checkpointIteration + '.pkl', "rb") as f:
        my_data, eval_data, sol_data, NUMBER_OF_SLOTS, eval_data, iterations, actions_example, iteration, average_pred_value, highest_pred_value, cumulated_average_highest_pred_value, cumulated_average_pred, pred_value_iteration, lowestPositiveReward, stop_lowest_positive_Reward,  actions = pickle.load(f)
        vw = vowpalwabbit.pyvw.vw("-i checkpoints/vw_model_from_json_in_iterations_randomshuffle_"+checkpointIteration+".vow")
        print("Model Loaded")
...
while lowestPositiveReward < stop_lowest_positive_Reward:
    ...
    if LEARN:
        ...
        while search:
            randomNr = random.randint(0, len(my_data) - 1)
            count = 0
            while count < len(sol_data):
                if sol_data[count]["clusterCat"] == my_data[randomNr]["clusterCat"] \
                        and sol_data[count]["positiveRewards"] < stop_lowest_positive_Reward:
                    search = False
                    break
                count += 1
    ...
    pred_value = []
    slot_learn = ""
    do_learn = True #if false: only learn
    for slot_pred in pred:
        prededSlot = slot_pred[0][0]
        if sol_data[prededSlot]["clusterCat"] == my_data[randomNr]["clusterCat"]:
            cost = -1
            pred_value.append(cost * -1)
            sol_data[prededSlot]['positiveRewards'] += 1
...
for t in eval_data:
    ...
    while is_random:
        ...
        is_random = False
        for slot_pred in pred:
            preded_slot = slot_pred[0]
            if preded_slot[1] < 0.2:
                print(preded_slot)
                is_random = True
                break
    print("GOT 4")
...
ax = plt.subplot()
sn.heatmap(df_cm, annot=True, fmt='g', ax=ax, xticklabels=True,
           yticklabels=True);  # annot=True to annotate cells, ftm='g' to disable scientific notation
