...
checkpointIteration=50000
if load_from_checkpoint:
    vw = vowpalwabbit.pyvw.vw(
        "-i my_clust_checkpoint/vw_model_from_json_in_iterations_randomshuffle_" + str(checkpointIteration) + ".vow")
    with open('my_clust_checkpoint/pickle_' + str(checkpointIteration) + '.pkl', 'rb') as f:
        my_data, eval_data, NUMBER_OF_SLOTS, eval_data, iterations, actions_example, iteration, average_pred_value, highest_pred_value, cumulated_average_highest_pred_value, cumulated_average_pred, pred_value_iteration, lowestPositiveReward, stop_lowest_positive_Reward,  actions = pickle.load(f) # lade daten, welche gemäß der txt Datei überarbeitet wurden sind

else:
    with open("my_clustTraining.json", "r", encoding="utf8") as f:
        my_data = json.load(f)
    ...
    if LEARN:
        ...
        vw = vowpalwabbit.pyvw.vw("-i my_clust_checkpoint/vw_model_from_json_in_iterations_randomshuffle.vow")
        print("Model Loaded")
...
while lowestPositiveReward < stop_lowest_positive_Reward:
    ...
    for v in pred_value:
        ...
        else: pred_average += v * 1/NUMBER_OF_SLOTS
        c += 1
    ...
    if (iteration % 500 == 0):
        ...
        vw.save('my_clust_checkpoint/vw_model_from_json_in_iterations_randomshuffle_' + str(iteration) + '.vow')
        pickle_files = [my_data,
                        ...
                        stop_lowest_positive_Reward,
                        actions]
        with open('my_clust_checkpoint/pickle_' + str(iteration) + '.pkl', 'wb') as f:
...
plt.show()
if LEARN:
    vw.save('my_clust_checkpoint/vw_model_from_json_in_iterations_randomshuffle_done.vow')
    vw.finish()
...
ax = plt.subplot()
sn.heatmap(df_cm, annot=True, fmt='g', ax=ax, xticklabels=True, yticklabels=True);  # annot=True to annotate cells, ftm='g' to disable scientific notation
